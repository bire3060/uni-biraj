...
const degrees = [
  { title: "On Campus", val: "OC" },
  { title: "Online", val: "ON" },
  { title: "Both", val: "BO" },
];

const Mode = ({ setActive, setStep }) => {
  const dispatch = useDispatch();
  const setStudyMode = (data) => {
    dispatch({
      type: ADD_GET_STARTED_FILTER,
      payload: data.val,
      property: "study_mode_started",
    });
  ...
  return (
    <div className="space-y-10  py-6">
      <Header text="Which study mode you want to pursue?" />
      ...
      {/* <Proceed setActive={() => setActive("duration")} step={4} /> */}
    </div>
  );
};
