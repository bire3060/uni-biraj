...
import { studyLoadValue, studyModeValue } from "../../../redux/actions/study";
import ClearStudyMode from "../StudyRadio/ClearStudyMode";
import ClearStudyLoad from "../StudyRadio/ClearStudyLoad";
import DataLoader from "../../common/Loader";
import { rangeSliderHandler } from "../../../redux/actions/rangeSliderHandler";
import RangeSlider from "../../../components/home/slider/range-slider/range-slider";
...
const CourseFilter = () => {
  ...
  const { country_city, country_header } = useSelector(
    (state) => state.countryCourseFilter
  );
  ...
  const handleExpandOrChange = (index, i, filter, property) => {
    // setting the pagination current button to 1 when filtering is done
    ...
    axiosInstance
      ...
      .then((res) => {
        ...
        dataArray.forEach((arr) => {
          ...
          } else if (arr.header === country_header) {
            arr.options.forEach((data) => {
              data.checked = true;
            });
          }
        ...
        if (country_header !== "") {
          for (let i = 0; i < country_city.length; i++) {
            dispatch(actions.add_country(country_city[i]));
          }
          dispatch(actions.add_count("city_count", count + 1));
          setTimeout(() => {
  ...
  const getCategoryList = (signal) => {
    axiosInstance
      ...
      .then((res) => {
        ...
        if (cat_category !== "") {
          dispatch(actions.add_categories(cat_category));
          dispatch(actions.add_count("categories_count", res.data.length + 1));
          setTimeout(() => {
