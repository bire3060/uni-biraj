...
const UserAgreement = ({ loggedIn, role }) => {
  const [loader, setLoader] = useState(true);
  const [getAllUserAgreement, setGetAllUserAgreement] = useState({});

  // getting all user agreement
  const getAllContacts = (signal) => {
    axiosInstance
      .get("/settings/detail-cms/user-agreement", { signal })
      .then((res) => {
        setLoader(false);
        setGetAllUserAgreement(res.data);
      })
      .catch((err) => {
  ...
  const { description, image } = getAllUserAgreement;
